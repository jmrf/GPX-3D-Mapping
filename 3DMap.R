...
sprintf("Reading GPX file: %s", input_gpx_file)

gpx.df <- readGPX(input_gpx_file)
gpx <- gpx.df$tracks |> unlist(recursive = FALSE) |> as.data.frame()
...
elev_img <-
  get_elev_raster(ex.df,
                  ...
                  clip = "bbox")
elev_tif <-
  raster::writeRaster(elev_img, "elevation.tif", overwrite = TRUE)
dim <- dim(elev_tif)
elev_matrix <- matrix(
...
for (i in 1:36) {
  rgl::lines3d(
    ...
    z[1:ceiling((1555 / 360) * i)] / (zscale - .05),-y[1:ceiling((1555 /
                                                                    360) * i)],
    color = "red",
    lwd = 4,
    smooth = T,
